...
contains
  ...
  subroutine math_lRMSD(coor1, coor2, rmsd, ifrotatecoor1)
    real*8,intent(inout) :: coor1(:),coor2(:)
    real*8,intent(out) :: rmsd
    logical, intent(in), optional :: ifrotatecoor1
    integer :: inum,i,j,idim, coordim, info
    real*8 :: tpvec(3),temp,umatrix(3,3),vmatrix(3,3),wvector(3),tpmatrix(3,3), work(15)
    real*8 :: originmatrix(3,3),dsign
    real*8, allocatable :: tpcoor(:)

    coordim = size(coor1, 1)
    allocate(tpcoor(coordim))

    if ( mod(coordim,3) /= 0 ) then
       write(*,*) "wrong ndim!"
       stop
    end if
    ...
    tpvec(1:3)=0.0d0
    do idim=1,coordim,3
      tpvec(1:3)=tpvec(1:3)+coor1(idim:idim+2)
    end do
    tpvec(1:3)=tpvec(1:3)/dble(coordim/3)
    do idim=1,coordim,3
      coor1(idim:idim+2)=coor1(idim:idim+2)-tpvec(1:3)
    end do
    tpvec(1:3)=0.0d0
    do idim=1,coordim,3
      tpvec(1:3)=tpvec(1:3)+coor2(idim:idim+2)
    end do
    tpvec(1:3)=tpvec(1:3)/dble(coordim/3)

    do idim=1,coordim,3
      coor2(idim:idim+2)=coor2(idim:idim+2)-tpvec(1:3)
    end do
